...
public class PublicMethod {
  ...
  public static Boolean sendcoin(byte[] to, long amount, byte[] owner, String priKey,
    ...
    while (times++ <= 2) {
      ...
      BalanceContract.TransferContract contract = builder.build();
      Protocol.Transaction transaction = wallet.createTransaction(contract);
      if (transaction == null || transaction.getRawData().getContractCount() == 0) {
        continue;
      }
      transaction = signTransaction(ecKey, transaction);
      GrpcAPI.Return response = broadcastTransaction(transaction, wallet);
      return response.getResult();
    }
    return false;
  }
   ...
   */
  public static GrpcAPI.Return broadcastTransaction(
          Protocol.Transaction transaction, Wallet wallet) {
    int i = 10;
    GrpcAPI.Return response = wallet.broadcastTransaction(transaction);
    while (!response.getResult() && response.getCode() == GrpcAPI.Return.response_code.SERVER_BUSY
            && i > 0) {
      try {
        Thread.sleep(300);
      ...
      i--;
      response = wallet.broadcastTransaction(transaction);
    }
    return response;
  }
}
